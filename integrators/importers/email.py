...
__all__ = ['IMAPClient', 'part_to_str', 'DEFAULT_GMAIL_HOST', 'DEFAULT_GMAIL_INBOX', 'DEFAULT_PORT',
           'get_unique_accounts', 'get_g_attr', 'create_external_id', 'split_external_id',
           'get_external_ids_in_namespace', 'EmailImporter']
...
# TODO: both these function should be done at the item-level\n",
def create_external_id(namespaces, external_id):
    return ":".join(namespaces) + ":" + external_id

def split_external_id(external_id):
    return external_id.split(":")

def get_external_ids_in_namespace(external_ids, namespace):
    # Return all ids within a namespace, e.g. ['c', 'd'] for ['a', 'b', 'c', 'd']
    # if the namespace is ['a', 'b']
    return [i[len(namespace):] for i in external_ids if i[0:len(namespace)] == namespace]
...
class EmailImporter(ImporterBase):
    ...
    def create_item_from_mail(self, mail, account_id, mailbox, message_uid, thread_id=None):
        ...
        external_id = create_external_id(['MAIL', account_id, mailbox], message_uid.decode("utf-8"))
        ...
        email_item = EmailMessage(externalId=external_id, subject=subject, dateSent=timestamp, content=content)
    ...
    def get_existing_ids(self):
        temp = [item for item in pod_client.get_all_items() if item.externalId != None]
        temp = [split_external_id(item.externalId) for item in temp]
        temp = get_external_ids_in_namespace(temp, ['MAIL'])
        # TODO: check if account and mailbox match
        return [bytes(i[-1], 'utf-8') for i in temp]

    @staticmethod
    def filter_existing_ids(mail_ids, existing_ids):
        existing_ids = set(existing_ids)
        return [x for x in mail_ids if x not in existing_ids]
    ...
    def get_mails(self, mail_ids, batch_size=5, importer_run=None, verbose=True, pod_client=None):
        ...
        for i, batch_ids in enumerate(batch(mail_ids, n=batch_size)):
            ...
            for mail, message_uid, thread_id in self.imap_client.get_mails(mail_ids):
                item = self.create_item_from_mail(mail,
                                                  self.imap_client.username,
                                                  self.imap_client.inbox,
                                                  message_uid,
                                                  thread_id=thread_id)
                # TODO: this conditional seems wrong?
    ...
    def run(self, importer_run, pod_client=None, verbose=True):
        ...
        mail_ids = self.filter_existing_ids(mail_ids, existing_ids)
