...
__all__ = ['IMAPClient', 'part_to_str', 'DEFAULT_GMAIL_HOST', 'DEFAULT_GMAIL_INBOX', 'merge_duplicate_items',
           'get_g_attr', 'GmailImporter']
...
class IMAPClient():
    ...
    def __init__(self, username, app_pw, host=DEFAULT_GMAIL_HOST, port=993, inbox=DEFAULT_GMAIL_INBOX):
        # Quick fix to support Google's threading method
        if host == DEFAULT_GMAIL_HOST:
...
class GmailImporter(ImporterBase):
    ...
    def set_imap_client(self, importer_run):
        imap_host = get_g_attr(importer_run, 'host', 'string')
        port = get_g_attr(importer_run, 'port', 'int')
    ...
    @staticmethod
    def get_content(email_message_instance):
